...
function Map({ data, homes, filter, setMyPosition }) {
  ...
  const [mapCenter, setMapCenter] = useState({
    lat: 40.417163732639686,
    lng: -3.705418109893799,
  });
  ...
  return isLoaded ? (
    <main
      className="map--container" /* style={{width:"100vw",height:"90vh"}} */
    >
      <GoogleMap
        ...
        <div className="event--modal">
          ...
          <div className="event--modal-info">
            {Object.keys(modalInfo).map((e, index) => (
              <span className="event--modal-list-labels" key={`label${index}`}>
                {t(`forms.${e}`)}
              </span>
            ))}
            {Object.values(modalInfo).map((e, index) =>
              e === 0 || e === 1 ? (
                <span
                  className="event--modal-list-values"
                  key={`value${index}`}
                >
                  {e === 1 ? t(`forms.yes`) : t(`forms.no`)}
                </span>
              ) : (
                <span
                  className="event--modal-list-values"
          ...
          {modalInfo.capacity ? (
            <button
              type="button"
              onClick={(e) => navigate(`/events/detail/${markerId}`)}
            >
              + info
            </button>
          ) : (
            <button
              type="button"
              onClick={(e) => navigate(`/events/center/${markerId}`)}
            >
              + info
            </button>
          )}
